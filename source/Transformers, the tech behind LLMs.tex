...
\title{Transformers, the tech behind LLMs}
\author{}
\date{}
...
GPT, you see, stands for \textbf{Generative Pretrained \textit{Transformer}}.

The meaning of ``generative'' here is fairly obvious, as ChatGPT \textit{generates} content in response to an input prompt. So is the meaning of ``pretrained'', if you've followed the 3Blue1Brown videos and my articles up to now. ChatGPT is essentially a machine with lots of knobs and dials parameters that have been gradually tweaked and tuned into an optimal configuration during a \textit{training} process.
...
\section*{Predict, sample, repeat}

A transformer is a function with the following input and output:

\begin{itemize}
	\item \textbf{Input:} a sequence of words
	\item \textbf{Output:} a ranking of all possible words that could come after the input, in terms of how likely they are to be the correct next word\footnote{In technical jargon, the output is a \textit{probability distribution over all possible next words}.}
\end{itemize}

To build a chatbot like ChatGPT from a transformer, one uses the following simple process:

\begin{enumerate}
	\item \textbf{(Predict).} Apply the transformer to the input prompt and obtain the ranking of all possible next words.
	\item \textbf{(Sample).} Take a random sample of ranked words, and append the most likely of these words to the input prompt.
	\item \textbf{(Repeat).} Repeat the process on the modified input prompt.
\end{enumerate}
...
The only additional step is to ensure that a so-called \textit{system prompt}, which tells the transformer to imagine what a conversation between an AI assistant and a user \textit{would} look like, is appended to every prompt the user sends ChatGPT behind the scenes. A system prompt might look like this:
...
\begin{enumerate}
	\item \textbf{(Tokenization).} First, the input is broken up into a bunch of little pieces called \textit{tokens}. In the case of text, tokens might be sequences of characters separated by delimiters--- it's accurate enough to simply imagine them being words. (In the case of images or audio, tokens might be little pieces of an overall image, or little pieces of the overall audio clip.)
	\item \textbf{(Embedding).} Each token is associated with a vector (a list of numbers) in such a way that tokens with similar meaning correspond to vectors that, when interpreted as locations in a higher-dimensional space, are close to each other. (Later on in the video, we learn that we like to think of the act of associating tokens with vectors as ``embedding'' the tokens into a ``space'' of vectors.)
	\item \textbf{(Attention).} All of these vectors are acted on by an \textit{attention} operation. The output of the attention operation is a list of transformed vectors whose meanings have been refined by the meanings of the other input vectors they appear near. It can be helpful to interpret the input vectors as being the columns of a matrix, and to imagine the attention operation as a sort of physical machine that the input matrix moves through before proceeding to the next machine.
	\item \textbf{(Feedforward).} That next ``machine'' is a neural network referred to as a \textit{feedforward layer}. Output vectors from the attention operation are acted on in parallel, and independently of each other, by this feedforward layer, which is sometimes also called a \textit{multilayer perceptron}, or \textit{MLP}.
	\item \textbf{(Repeat).} The process repeats many times. The output vectors from the previous step are passed through more and more attention-block-feedforward-layer pairs.
\end{enumerate}

The third step, \textit{attention}, is the core idea at the heart of transformers. It was aptly featured in the title of the seminal 2017 Google research paper that introduced transformers, \textit{Attention is All You Need}.

Since attention is a relatively complicated mechanism, Grant saves the discussion of exactly how it works for the next video. The current video and article are dedicated to explaining everything else--- some background on word embeddings, what happens before attention, what the goal of attention is, and what happens after attention. This ``everything else'' would have been familiar to any machine learning engineer when \textit{Attention is All You Need} was published, so it's worth taking time to familiarize ourselves with it.

First, let's go over some background.

\section*{Background: word embeddings}
...
Specifically, it was already possible to have a word representation function that maps every word to a vector (a list of numbers)\footnote{Google's Word2Vec, presented in the \textit{Efficient Estimation of Word Representations in Vector Space}, was the first such library providing this functionality.}.

Since a vector of $n$ numbers can interpreted to the coordinates of a point in $n$-dimensional space, we can think of a word representation function as mapping words to locations, or equivalently, directions (since every location corresponds to the direction between it and the origin). And, if we conflate the words with their representations, pretending that a word \textit{is} its representation, we might suggestively say that that a word representation function \textit{embeds} words into that space; it takes the hazy, abstract notions of human words, and grounds them in the more tangible reality of lists of numbers.

For these metaphorical reasons, we have the following terminology:

\begin{itemize}
	\item The word representation function (the function sending words to vectors) is known as an \textit{embedding}.
	\item A vector representing a word is also sometimes called an \textit{embedding}. (So, you have to use context to figure out whether ``word representation function'' or ``representation of a word'' is meant!)
	\item The set of all possible word representation is called the \textit{embedding space}.
\end{itemize}

\subsection*{Mirrored relationships}

The exciting idea to represent words with vectors is only actually useful if the relationships between vectors somehow \textit{mirror} the relationships between the corresponding words. For example, we might have the prior impression that the semantic difference between the words ``woman'' and ``man'' is about the same as the semantic difference between ``queen'' and ``king''.
$$
\text{woman} - \text{man} \approx \text{queen} - \text{king}
$$
Of course, it's not really clear how one is supposed to subtract ``man'' from ``woman''. However, if we could take the embedding vector of ``man'', we \textit{could} subtract that from the embedding vector of ``woman''. And we could do the same for ``queen'' and ``king''. So, if $\mathbf{E}$ is the embedding function, we might expect that we would have something like
$$
...
At this point, the obvious question is--- how in the world was the machine learning community able to come up with embedding functions that have this mirroring property? The answer is \textit{training}. The embedding function is secretly a neural network that takes words as input, produces vectors as output, and that has been trained to do this mapping in a way that ensures mirroring of relationships.

\subsection*{Dot product semantics}
...
\begin{quote}
There is an operation between two vectors $\vv$ and $\ww$ that is denoted by $\vv \cdot \ww$, that is read out loud as ``v dot w'', and is defined to be the result of multiplying the length of the projection (the ``shadow'') of $\vv$ onto $\ww$ by the length of $\ww$. Since the dot product of two vectors is a product of lengths, it is a number. When we have vectors $\vv = \begin{pmatrix} v_1 \\ \vdots \\ v_n \end{pmatrix}$ and $\ww = \begin{pmatrix} w_1 \\ \vdots \\ w_n \end{pmatrix}$, it is possible to prove that the dot product of $\vv$ and $\ww$ is equal to the result of multiplying corresponding components and summing them: $\vv \cdot \ww = v_1 w_1 + \ldots + v_n w_n$.
\end{quote}

For the full details, you can see \href{https://github.com/rossgk2/Linear-algebra-tensors-manifolds}{my book on linear algebra, tensors, and manifolds}. For our purposes, though, all that need be understood is that the dot product of two vectors is an easily computable measure of how much they align.
...
We can formalize this idea with embeddings by defining a \textit{plurality embedding} to be a difference of any plural embedding (like ``cats'') and its corresponding singular embedding (``cat''):
$$
...
\section*{The steps in a transformer}
...
\subsection*{Before attention: tokenization and embedding}

\textbf{(Tokenization).} First, the input is first broken up into a bunch of little pieces called \textit{tokens}. In our example, the tokens might be ``Harry'', ``Potter,'', ``the'', ``boy'', ``who'', ``lived''.

\textbf{(Embedding).} We map the tokens to embedded vectors by using the \textit{embedding matrix}, which has one column vector for every word in the dictionary. The embedding matrix is denoted by $\mathbf{W}_E$, and was learned during training in such a way that it ensures the semantic ``mirroring'' we discussed earlier.
...
\subsection*{The goal of attention: embeddings beyond words}

\textbf{(Attention, feedforward, repeat).} When our matrix $\mathbf{X}$ of embedded vectors arrives at the attention step, its first column vector, ``Harry'', points in whatever direction in the embedding space is associated with traditional British male names. In fact, \textit{all} of the embedded vectors in the matrix represent mere, simple words. But, precisely because they reside in a high-dimensional space, they have the \textit{capacity} to soak in so much more context than this.
...
\subsubsection*{The unembedding matrix}

\textbf{(Unembedding, continued).} One convenient way of computing the above vector of dot products is by taking the matrix whose rows are the possible words, and multiplying it by the column vector $\mathbf{E}(\text{who})$:
$$
\begin{pmatrix}
...
\mathbf{E}(\text{who})
$$
This matrix is called the \textit{unembedding matrix}, and it is denoted $\mathbf{W}_U$. Like the embedding matrix, it is learned from training.

You might be wondering--- why can't we just use the columns of the embedding matrix as the rows of the unembedding matrix? Don't both sets of vectors represent the set of all possible words? Well, yes. And some models indeed do this. But it's a common enough choice to maintain entirely separate matrices for embedding and unembedding. The difference between \textit{representing} a word and \textit{predicting} the next word is enough that doing so can lead to substantial model improvement.

\subsubsection*{Softmax}

\textbf{(Unembedding, continued).} Back to our vector of dot products:
$$
\begin{pmatrix}
    -1.500 & | & \text{aah} \\
    -1.500 & | & \text{aardvark} \\
...
\subsubsection*{Extra: softmax with temperature}
...
$$
Since this parameter plays a similar role as temperature does in thermodynamics equations from physics, it is referred to as the \textit{temperature}.
