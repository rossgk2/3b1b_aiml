...
\begin{document}
	...
	In this article, we present how backpropagation is actually done in practice. With the goal of determining the gradient $\nabla c_k$, we'll compute the partial derivatives of $c_k$ with respect to an arbitrary weight and an arbitrary bias in an arbitrary layer (layer $\ell$), and find that they recursively depend on the same partial derivatives for layer $\ell + 1$. Then we'll determine concrete non-recursive expressions for these partial derivatives in layer $L$. The \textit{backpropagation equations} we obtain recursively determine the partial derivatives for all layers, and therefore determine the gradient $\nabla c_k$.
	
	\subsection*{Notation warning}
	
	An important heads up! We will actually use $c_i$ to speak of the cost function for the $i$th training example, rather than $c_k$ to speak of the cost function for the $k$th training example. This is because we want to reserve the letter $k$ so that we can use it to denote the weight $w^{(\ell)}_{jk}$ from the $j$th neuron in layer $\ell - 1$ to the $k$th neuron in layer $\ell$.
	...
	\begin{itemize}
		\item $c_i$ is the cost function for the $i$th training example
		\item $w^{(\ell)}_{jk}$ is the weight from the $j$th neuron in layer $\ell - 1$ to the $k$th neuron in layer $\ell$
		...
		\begin{align*}
			z^{(\ell)}_j := \sum_{r = 1}^{n_\ell} w^{(\ell)}_{rj} a^{(\ell - 1)}_r + b^{(\ell)}_j
		\end{align*}
	...
	\begin{align*}
		\frac{\partial c_i}{\partial w^{(\ell)}_{jk}} &= \frac{\partial c_i}{\partial a^{(\ell)}_j} \frac{\partial a^{(\ell)}_j}{\partial z^{(\ell)}_j} \frac{\partial z^{(\ell)}_j}{\partial w^{(\ell)}_{jk}}, \\
	...
	\begin{align*}
		\frac{\partial c_i}{\partial a^{(\ell)}_j} &= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} \frac{\partial z^{(\ell + 1)}_r}{\partial a^{(\ell)}_j} \\
		&= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} \frac{\partial}{\partial a^{(\ell)}_j}\left(\sum_{s = 1}^{n_\ell} w^{(\ell + 1)}_{sr} a^{(\ell)}_s + b^{(\ell + 1)}_r\right) \\
		&= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} \left(\sum_{s = 1}^{n_\ell} \frac{\partial}{\partial a^{(\ell)}_j}\left(w^{(\ell + 1)}_{sr} a^{(\ell)}_s\right) + \frac{\partial b^{(\ell + 1)}_r}{\partial a^{(\ell)}_j}\right) \\
		&= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} \left(\sum_{s = 1}^{n_\ell} w^{(\ell + 1)}_{sr} \frac{\partial}{\partial a^{(\ell)}_j} \left( a^{(\ell)}_s \right) \right) \\
		&= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} \left(\sum_{s = 1}^{n_\ell} w^{(\ell + 1)}_{sr} \delta_{sj} \right), \text{ where $\delta_{sj} = 1$ when $s = j$ and $\delta_{sj} = 0$ when $s \neq j$} \\
		&= \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} w^{(\ell + 1)}_{jr}.
	\end{align*}
	
	Notice how recursion has already crept in. The above expression involves the partial derivative $\frac{\partial c_i}{\partial z^{(\ell + 1)}_r}$, which is only known after computations for the next layer, layer $\ell + 1$, have been done.
	...
	\begin{align*}
		\frac{\partial z^{(\ell)}_j}{\partial w^{(\ell)}_{jk}} &=  \frac{\partial}{\partial w^{(\ell)}_{jk}}\left(\sum_{r = 1}^{n_{\ell-1}} w^{(\ell)}_{rj} a^{(\ell-1)}_r + b^{(\ell)}_j\right) \\
		&= \left(\sum_{r = 1}^{n_{\ell-1}} \frac{\partial}{\partial w^{(\ell)}_{jk}}\left(w^{(\ell)}_{rj} a^{(\ell-1)}_r\right) + \frac{\partial b^{(\ell)}_j}{\partial w^{(\ell)}_{jk}}\right) \\
		&= \left(\sum_{r = 1}^{n_{\ell-1}} \frac{\partial}{\partial w^{(\ell)}_{jk}}\left(w^{(\ell)}_{rj}\right) a^{(\ell-1)}_r \right) \\
	...
	\begin{align*}
		&\frac{\partial c_i}{\partial w^{(\ell)}_{jk}} = \frac{\partial c_i}{\partial a^{(\ell)}_j} \frac{\partial a^{(\ell)}_j}{\partial z^{(\ell)}_j} \frac{\partial z^{(\ell)}_j}{\partial w^{(\ell)}_{jk}} = \frac{\partial c_i}{\partial a^{(\ell)}_j} \sigma'(z^{(\ell)}_j) a^{(\ell - 1)}_k, \\
		...
		&\text{where } \frac{\partial c_i}{\partial a^{(\ell)}_j} = \sum_{r = 1}^{n_{\ell+1}} \frac{\partial c_i}{\partial z^{(\ell + 1)}_r} w^{(\ell + 1)}_{jr}.
	\end{align*}
	
	Again, notice recursion in the expression for $\frac{\partial c_i}{\partial a^{(\ell)}_j}$. In order to compute $\frac{\partial c_i}{\partial a^{(\ell)}_j}$, we must already know its value for the next layer.
	...
	We compute the partial derivatives for layer $L$ now.
	
	\begin{align*}
		\frac{\partial c_i}{\partial w^{(L)}_{jk}} &= \frac{\partial c_i}{\partial a^{(L)}_j} \frac{\partial a^{(L)}_j}{\partial z^{(L)}_j} \frac{\partial z^{(L)}_j}{\partial w^{(L)}_{jk}}, \\
	...
	And just as before, we compute the partial derivatives appearing in the products. First, we have
	
	\begin{align*}
		\frac{\partial c_i}{\partial a^{(L)}_j} &= \frac{\partial}{\partial a^{(L)}_j} \sum_{p = 1}^{n_L} (y_{ip} - a^{(L)}_p)^2 \\
		&= \sum_{p = 1}^{n_L} 2(y_{ip} - a^{(L)}_p)\frac{\partial}{\partial a^{(L)}_j}\left(y_{ip} - a^{(L)}_p\right) \\
		&= \sum_{p = 1}^{n_L} 2(y_{ip} - a^{(L)}_p)(-\delta_{pj})\text{, where $\delta_{pj} = 1$ when $p = j$ and $\delta_{pj} = 0$ when $p \neq j$} \\
		&= 2(a^{(L)}_j - y_{ij}).
	\end{align*}
	...
	\begin{align*}
		\frac{\partial c_i}{\partial w^{(L)}_{jk}} &= \frac{\partial c_i}{\partial a^{(L)}_j} \frac{\partial a^{(L)}_j}{\partial z^{(L)}_j} \frac{\partial z^{(L)}_j}{\partial w^{(L)}_{jk}} = 2(a^{(L)}_j - y_{ij}) \sigma'(z^{(L)}_j) a^{(L - 1)}_k, \\
	...
	Of course, the expression for $\frac{\partial c_i}{\partial a^{(\ell)}_j}$ is recursive. In order to know its value for some neuron in layer $\ell$, we must already know $\frac{\partial c_i}{\partial z^{(\ell + 1)}_r}$ for all $r$, which\footnote{Since $\frac{\partial c_i}{\partial z^{(\ell + 1)}_r} = \frac{\partial c_i}{\partial a^{(\ell + 1)}_r} \frac{\partial a^{(\ell + 1)}_r}{\partial z^{(\ell + 1)}_r} = \frac{\partial c_i}{\partial a^{(\ell + 1)}_r} \sigma'(z^{(\ell + 1)}_r)$.} can be determined from knowing $\frac{\partial c_i}{\partial a^{(\ell)}_r}$ for all neurons in layer $\ell + 1$. This is fine, of course, since we know how to compute its value in the last layer, layer $L$. Knowledge of this partial derivative in layer $L$ layer can thus be ``propagated back'' from layer $L$ to layer $L - 1$, and from layer $L - 1$ to layer $L - 2$, and from layer $L - 2$ to layer $L - 3$, until finally it is known for all layers.
	...
	\begin{align*}
		&\frac{\partial c_i}{\partial w^{(\ell)}_{jk}} = \delta^{(\ell)}_j a^{(\ell - 1)}_k, \\
